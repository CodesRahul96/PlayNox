package com.codesrahul.playnox.ui.screen.editor
    ...
    fun provideCompletion(prefix: String, mode: MpvCompletionMode, publisher: CompletionPublisher) {
        ...
        val suggestions = completionsFor(mode)
            ...
            .map { def ->
                ...
                val descWithDetail = def.detailText()?.let { "${def.desc} ($it)" } ?: def.desc
                SimpleCompletionItem(def.label, descWithDetail, prefix.length, commitText).apply {
                    kind(def.kind)
                }
            }
            .toList()
